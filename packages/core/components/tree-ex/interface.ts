...
import type { dragHoverPartEnum } from './core/constants'
import type TreeNode from './core/tree-node'
import type { FilterFunctionType } from './core/tree-store'
import type {
  AnyPropsArrayType,
  GetNodeFn,
  IgnoreType,
...
export interface TreeRootProps<T extends Record<string, any>> extends PrimitiveProps {
  ...
  nodeClassName?:
    ...
    | object
    | Array<string | object>
    | ((node: TreeNode<T>) => string | object | Array<string | object>)
...
//   >
// > &
//   Pick<TreeRootProps, 'render' | 'showLine'>

export type TreeItemProps<T extends Record<string, any>> = PrimitiveProps & {
  data: TreeNode<T>
  getNode?: GetNodeFn
