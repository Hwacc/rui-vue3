...
import type { IgnoreType, TreeKeyField, TreeNodeKeyType } from './types'
...
interface IMapData<T extends Record<string, any>> {
...
export type FilterFunctionType = <T extends Record<string, any>>(
  keyword: string,
  node: TreeNode<T>,
) => boolean

export default class TreeStore<T extends Record<string, any>> extends TreeEventTarget {
  // #region Properties
  ...
  /** 树数据 map */
  mapData: IMapData<T> = Object.create(null)
   ...
   */
  private insertIntoFlatData(insertIndex: number, insertNodes: TreeNode<T>[]) {
    this.flatData = this.flatData
      .slice(0, insertIndex)
      .concat(insertNodes, this.flatData.slice(insertIndex))
  }
  ...
  setData(
    ...
    this.data = data.map(
      (d): TreeNode<T> => new TreeNode(d, null, !!this.options.load, this.options.field),
    )
    // 清空 mapData
    for (const key in this.mapData) delete this.mapData[key]
  ...
  setChecked(
    ...
    if (node.checked && value)
      return // 已经勾选的再次勾选，直接返回
    if (!node.checked && !node.indeterminate && !value)
      return // 未勾选且不是半选状态再次设置未勾选，直接返回
  ...
  checkAll(triggerEvent: boolean = true, triggerDataChange: boolean = true): void {
    if (this.options.cascade) {
      ...
      const checkCascade = (nodes: TreeNode<T>[]): void => {
        nodes.forEach((node) => {
          if (node.disabled) {
   ...
   * @param triggerDataChange 是否触发视图刷新
   */
  private triggerCheckedChange(triggerEvent = true, triggerDataChange = true) {
    if (triggerEvent) {
   ...
   * @param triggerDataChange 是否触发视图刷新
   */
  private triggerSelectedChange(triggerEvent = true, triggerDataChange = true) {
    if (triggerEvent) {
  ...
  private isChildrenChanged(node: TreeNode<T>, newNode: ITreeNodeOptions): boolean {
    return 'children' in newNode && (!!node.children.length || !!newNode.children?.length)
  }

  updateNode(
    key: TreeNodeKeyType,
    newNode: ITreeNodeOptions,
    triggerEvent = true,
    ...
    const notAllowedFields = [this.options.field.id, 'indeterminate', 'visible', 'isLeaf']
    ...
    const triggerSetDataFlag = this.isChildrenChanged(node, newNodeCopy)
    ...
    if (triggerEvent) {
      if (
        JSON.stringify(currentCheckedKeys.sort()) !== JSON.stringify(previousCheckedKeys.sort())
      ) {
   ...
   */
  getCheckedNodes(ignoreMode = this.options.ignoreMode): TreeNode<T>[] {
    if (ignoreMode === ignoreEnum.children) {
      const result: TreeNode<T>[] = []
      const traversal = (nodes: TreeNode<T>[]) => {
        nodes.forEach((node) => {
          if (node.checked) {
            result.push(node)
          }
   ...
   * @param key 节点 key
   */
  getNode(key: TreeNodeKeyType): TreeNode<T> | null {
  ...
  private loadChildren(node: TreeNode<T>, children: any[], expand: boolean) {
  ...
  private insertIntoStore(
    ...
    const nodes: TreeNode<T>[] = this.flattenData([node])
    // 处理自身及子节点 _level
  ...
  filter(keyword: string, filterMethod: FilterFunctionType): void {
    // 使用树形结构数据进行遍历
    const filterVisibleNodes: TreeNode<T>[] = []
    this.flatData.forEach((node) => {
      node._filterVisible
  ...
  private setUnloadCheckedKeys(keys: TreeNodeKeyType[]): void {
    ...
    if (
      ...
      && newCheckedKeys.every(val => (checkedKeysCache as TreeNodeKeyType[]).includes(val))
    ) {
      return
    }
  ...
  private setUnloadSelectedKey(key: TreeNodeKeyType): void {
    ...
    if (newSelectedKey === selectedKeyCache)
      return
   ...
   */
  private checkNodeDownward(node: TreeNode<T>, value: boolean, filtering: boolean = false): void {
    ...
    if (node.isLeaf || (this.options.load && !node.children.length)) {
      if (!node.disabled) {
        ...
        if (filtering && !this.options.filteredNodeCheckable && !node._filterVisible) {
          return
        }
        node.checked = value
        node.indeterminate = false
   ...
   */
  private checkNodeUpward(node: TreeNode<T>, fromCurrentNode = false) {
   ...
   * @param node 需要勾选的节点
   */
  private checkParentNode(node: TreeNode<T>): void {
    if (!this.options.cascade)
      return
   ...
   */
  private findIndex(
    keyOrNode: TreeNode<T> | TreeNodeKeyType,
    searchList: TreeNode<T>[] | TreeNodeKeyType[] | null = this.flatData,
  ): number {
    if (searchList !== null) {
