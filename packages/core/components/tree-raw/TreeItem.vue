...
import { handleAndDispatchCustomEvent } from '@rui/core/lib/event'
import { flatten } from 'lodash-es'
import { Primitive, RovingFocusItem } from 'reka-ui'
import { computed } from 'vue'

import { injectTreeRootContext } from '.'
...
const isIndeterminate = computed(() => {
  if (
    rootContext.bubbleSelect.value
    && hasChildren.value
    && Array.isArray(rootContext.modelValue.value)
  ) {
    const children = flatten<T>(rootContext.getChildren(props.value) || [])

    return (
      children.some(child =>
        rootContext.modelValue.value.find(
          (v: any) => rootContext.getKey(v) === rootContext.getKey(child),
        ),
      )
...
function handleKeydownRight(ev: KeyboardEvent) {
  ...
  if (isExpanded.value) {
    ...
    const nextElement = list.find(
      el => Number(el.getAttribute('data-indent')) === props.level + 1,
    )
  ...
  >
    <Primitive
      ...
      @keydown.right.prevent="
        (ev) => (rootContext.dir.value === 'ltr' ? handleKeydownRight(ev) : handleKeydownLeft(ev))
      "
      @keydown.left.prevent="
        (ev) => (rootContext.dir.value === 'ltr' ? handleKeydownLeft(ev) : handleKeydownRight(ev))
      "
      @click.stop="
        (ev) => {
