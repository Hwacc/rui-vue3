import type { IgnoreType, TreeNodeKeyType } from '../types'
...
export default class TreeStore extends TreeEventTarget {
  ...
  updateNode(
    ...
    const notAllowedFields = [this.options.keyField, 'indeterminate', 'visible', 'isLeaf']
  ...
  private getInsertedNode(
    ...
    if (insertedNode instanceof TreeNode) {
      // 与参照节点是同一个节点
      if (insertedNode[this.options.keyField] === referenceKey)
        return null
      return insertedNode
    }
